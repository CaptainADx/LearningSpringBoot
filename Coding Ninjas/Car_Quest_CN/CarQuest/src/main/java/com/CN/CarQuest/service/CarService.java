...
public class CarService {
    ...
    public CarResponse getCarById(String name, String authorizationHeader) {
		...
		List<ReviewResponse> review = communicator.getReview(name, jwtToken);
		
		List<String> reviewList = new ArrayList<>();
		
		for(ReviewResponse res : review) {
			reviewList.add(res.getReview());
		}
		
		return CarResponse.builder().brand(car.getBrand()).color(car.getColor())
				.price(car.getPrice()).modelYear(car.getModelYear()).name(car.getName()).reviews(reviewList).build();
    }
