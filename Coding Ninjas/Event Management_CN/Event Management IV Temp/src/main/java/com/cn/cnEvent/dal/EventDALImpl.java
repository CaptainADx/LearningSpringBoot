...
public class EventDALImpl implements EventDAL {
	...
	public List<Ticket> getAllTicketsByEventId(Long eventId) {
		Session session  = entityManager.unwrap(Session.class);
		List<Ticket> allTickets = session.createQuery("From Ticket t Where t.event.id = :eventId",Ticket.class).setParameter("eventId", eventId).getResultList();
		
		return allTickets;
	}

	@Override
	public List<Event> getAllEventsByTicketPrice(Long price) {
		Session session  = entityManager.unwrap(Session.class);
		List<Event> allEvents = session.createQuery("select distinct e From Event e Join e.tickets t where t.price> :price", Event.class).setParameter("price", price).getResultList();
