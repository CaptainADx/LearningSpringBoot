...
public class PaymentDALImpl implements PaymentDAL{
	...
	public List<PaymentReview> getPaymentReviews(Integer paymentId) {
		...
		Session session = entityManager.unwrap(Session.class);
		List<PaymentReview> result = session.createQuery("FROM PaymentReview pr WHERE pr.payment.id = :paymentId", PaymentReview.class).setParameter("paymentId", paymentId).getResultList();
		return result;
	}
	...
	public List<Payment> getAllPaymentsByQueryType(String queryType) {
	    ...
	    return session.createQuery(
	            "SELECT DISTINCT p " +
	            "FROM PaymentReview pr JOIN pr.payment p " +
	            "WHERE pr.queryType = :queryType",
	            Payment.class)
