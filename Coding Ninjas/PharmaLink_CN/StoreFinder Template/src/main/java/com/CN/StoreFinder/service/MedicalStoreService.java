...
public class MedicalStoreService {
	...
	public List<MedicalStore> getNearestMedicalStores(Long userId, Long distance, String token) {
		
		return getAllMedicalStores().stream().filter(store -> calculateDistance(userId, store) <= distance).collect(Collectors.toList());
	}

	public List<MedicalStore> getMedicalStoresHavingMedicine(String medicine, String token) {
